...
class MuteConfig:
    ...
    sound_length_ms: int = 0             # обрезать звук до этой длины, 0 — как есть
    trim_silence: bool = True            # срезать тишину в начале и конце звука
    burst_tail_ms: int = 250             # держать заглушение после серии матов
...
class AIConfig:
    ...
    max_window_ms: int = 2000            # самое длинное окно анализа
